<template>
    <div class="tasklistsList">
        <h2 class="text-2xl font-semibold py-5">Coordinators List</h2>
                ...
                <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                    <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                        <table class="min-w-full leading-normal">
                            ...
                            <tbody>
                                <template v-for="(tasklist, tasklistIndex) in filtered">
                                    <template v-if="
                                        tasklistIndex >= paginationIndex &&
                                        tasklistIndex < paginationIndex + paginationStep
                                        ">
                                        <tr :key="tasklistIndex">
                                            <td class="px-5 py-5 bg-white text-sm">
                                                <div class="flex items-center">
                                                    <avatar :name="tasklist.user.full_name" :size="6" class="mr-3"></avatar>
                                                    <div class="ml-3">
                                                        <p class="text-gray-900 whitespace-no-wrap">
                                                            {{ tasklist.user.full_name }} </p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-5 py-5 bg-white text-sm">
                                            ...
                                            <td class="px-5 py-5 bg-white text-sm">
                                                <p class="text-gray-900 whitespace-no-wrap">
                                                    {{ tasklist.user.email }} </p>
                                            </td>

                                            <td v-if="$role === 'admin'"
                                                class="px-5 py-5 bg-white text-sm text-right">
                                                <a @click="editEmployee(tasklistIndex)"
...
export default {
    ...
    methods: {
        editEmployee(index) {
            this.eventHub.$emit("create-employees", this.filtered[index]);
        },
